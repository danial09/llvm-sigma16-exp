...
const Sigma16MCExpr *Sigma16MCExpr::create(const MCSymbol *Symbol,
...
const Sigma16MCExpr *
Sigma16MCExpr::createGpOff(Sigma16MCExpr::Sigma16ExprKind Kind,
                           const MCExpr *Expr, MCContext &Ctx) {
  return create(
      Kind, create(CEK_None, create(CEK_GlobalAddress, Expr, Ctx), Ctx), Ctx);
}
...
bool Sigma16MCExpr::evaluateAsRelocatableImpl(MCValue &Res,
...
bool Sigma16MCExpr::isGpOff(Sigma16ExprKind &Kind) const {
  if (const Sigma16MCExpr *S1 = dyn_cast<const Sigma16MCExpr>(getSubExpr())) {
    if (const Sigma16MCExpr *S2 =
            dyn_cast<const Sigma16MCExpr>(S1->getSubExpr())) {
