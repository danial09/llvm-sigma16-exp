...
concept __unqualified_iter_swap =
    ...
    requires(_T1&& __x, _T2&& __y) { iter_swap(_VSTD::forward<_T1>(__x), _VSTD::forward<_T2>(__y)); };
...
struct __fn {
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr void operator()(_T1&& __x, _T2&& __y) const noexcept(
      noexcept(iter_value_t<_T2>(ranges::iter_move(__y))) && noexcept(*__y = ranges::iter_move(__x)) && noexcept(
          *_VSTD::forward<_T1>(__x) = declval<iter_value_t<_T2>>())) {
