...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr drop_view(_View __base, range_difference_t<_View> __count)
...
struct __fn {
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI constexpr auto operator()(_Range&& __rng, _Np&& __n) const noexcept(noexcept(
    ...
    return _RawRange(ranges::begin(__rng) + clamped, ranges::end(__rng), std::__to_unsigned_like(dist - clamped));
  }
