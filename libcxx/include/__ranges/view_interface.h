...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr decltype(auto) front()
    requires forward_range<_D2>
  {
    _LIBCPP_ASSERT(!empty(), "Precondition `!empty()` not satisfied. `.front()` called on an empty view.");
  ...
  {
    _LIBCPP_ASSERT(!empty(), "Precondition `!empty()` not satisfied. `.back()` called on an empty view.");
