...
constexpr unscoped_enum iter_move(unscoped_enum& e) noexcept(false) {
    return e;
}

enum class scoped_enum { a, b, c };
constexpr scoped_enum* iter_move(scoped_enum&) noexcept {
    return nullptr;
}
...
constexpr int iter_move(some_union& u) noexcept(false) {
    return u.x;
}
...
public:
    ...
    constexpr move_tracker(move_tracker&& other) noexcept : moves_ {
        other.moves_ + 1
    } {
        other.moves_ = 0;
    }
    ...
    constexpr int moves() const noexcept {
        return moves_;
    }
