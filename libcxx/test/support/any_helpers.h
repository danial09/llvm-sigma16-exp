...
std::enable_if_t<has_value_member<Type>(0)>
    _LIBCPP_AVAILABILITY_THROW_BAD_ANY_CAST assertContains(std::any const& a, int value) {
  assert(a.has_value());
...
std::enable_if_t<!has_value_member<Type>(0)>
    _LIBCPP_AVAILABILITY_THROW_BAD_ANY_CAST assertContains(std::any const& a, Value value) {
  assert(a.has_value());
  assert(containsType<Type>(a));
...
// 'value'.
template <class Type>
_LIBCPP_AVAILABILITY_THROW_BAD_ANY_CAST void modifyValue(std::any& a, int value) {
  assert(a.has_value());
  assert(containsType<Type>(a));
...
struct small_tracked_t {
  ...
  small_tracked_t(small_tracked_t const&) noexcept : arg_types(&makeArgumentID<small_tracked_t const&>()) {}
  small_tracked_t(small_tracked_t&&) noexcept : arg_types(&makeArgumentID<small_tracked_t&&>()) {}
  template <class... Args>
...
struct large_tracked_t {
  ...
  large_tracked_t(large_tracked_t const&) noexcept : arg_types(&makeArgumentID<large_tracked_t const&>()) {}
  large_tracked_t(large_tracked_t&&) noexcept : arg_types(&makeArgumentID<large_tracked_t&&>()) {}
  template <class... Args>
