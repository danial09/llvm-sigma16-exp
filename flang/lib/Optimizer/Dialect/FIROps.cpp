...
void fir::AllocaOp::build(mlir::OpBuilder &builder,
                          ...
                          llvm::ArrayRef<mlir::NamedAttribute> attributes) {
    auto nameAttr =
        uniqName.empty() ? mlir::StringAttr{} :
        builder.getStringAttr(uniqName);
    auto bindcAttr =
        bindcName.empty() ? mlir::StringAttr{} :
...
void fir::CmpcOp::print(mlir::OpAsmPrinter &p) {
    printCmpOp(p, *this);
}
...
static void printCustomRangeSubscript(mlir::OpAsmPrinter &printer,
    ...
    llvm::interleaveComma(
        make_filter_range(
            enumerate,
    [](auto indexed_value) {
        return indexed_value.index() % 2 == 0;
    }),
    printer, [&](auto indexed_value) {
        printer << indexed_value.value();
    });
    printer << ") to (";
...
static bool isZero(mlir::Attribute a) {
    return checkIsIntegerConstant(a, 0);
}
static bool isOne(mlir::Attribute a) {
    return checkIsIntegerConstant(a, 1);
}
...
mlir::Region &fir::DoLoopOp::getLoopBody() {
    return getRegion();
}
...
bool fir::StringLitOp::isWideValue() {
    return stringLitOpGetKind(*this) != 1;
}
